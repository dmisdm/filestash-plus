...
const $tmpl = createElement(`
    <a href="__TEMPLATE__" class="component_thing no-select" data-selectable="true" draggable="false" data-link>
        ...
        <div class="component_action">
            <span><img class="component_icon" draggable="false" src="${IMAGE.DOWNLOAD}" alt="download"></span>
        </div>
...
export function createThing({
    ...
    const $action = $thing.children[5]; // = qs($thing, ".component_action");
    ...
    if (link) {
        ...
        $thing.classList.add("no-navigate");
    }
    ...
    if (type === "file") {
        $action.onclick = (e) => triggerDownload(e, path, name);
    } else {
...
function triggerDownload(e, path, name) {
    e.preventDefault();
    e.stopPropagation();
    let href = isDir(path)
        ? `api/files/zip?path=${encodeURIComponent(path)}`
        : `api/files/cat?path=${encodeURIComponent(path)}&mode=download&name=${encodeURIComponent(name)}`;
    href = forwardURLParams(href, ["share", "canary"]);
    const $link = document.createElement("a");
    $link.href = href;
    $link.download = isDir(path) ? name + ".zip" : name;
    $link.click();
    $link.remove();
}
